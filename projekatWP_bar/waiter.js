...
export class Waiter
{
    ...
    drawForm(host){
        ...
        something.innerHTML="Konobar: "+this.name;
        ...
        something.innerHTML="Ukupno: "+this.finalPrice;
        contForm2.appendChild(something);
    ...
    newOrder()
    {
        ...
        fetch("https://localhost:5001/Bar/UpdateOrder/"+this.name,{
            method:'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
            body:JSON.stringify(this.currentOrders[this.num_order-1])
        });
        ...
        let pom=this.container.querySelector("#ukupna_cena");
        pom.innerHTML="Ukupno: 0";
    }
    ...
    deleteSpec(a)
    {
        ...
        this.currentOrders[this.num_order-1].deleteGlass(a);
        this.currentDrinks=this.currentOrders[this.num_order-1].glasses().map((x)=>x);
        this.glasses--;
        this.finalPrice-=specPrice;

        let row=this.container.querySelector("#row1");
        row.deleteCell(a-1);

        row=this.container.querySelector("#row2");
        row.deleteCell(a-1);

        for(let i=a-1;i<this.glasses;i++){
            this.container.querySelector("#canvas"+(i+1)).id="canvas"+i;
            this.container.querySelector("#th"+(i+1)).id="th"+i;
        }

        let pom=this.container.querySelector("#ukupna_cena");
        pom.innerHTML="Ukupno: "+this.finalPrice;
    }
    ...
    deleteAll()
    {
        ...
        this.finalPrice-=this.currentOrders[this.num_order-1].price;
        this.currentOrders.pop();
        this.currentDrinks=[];
        this.glasses=0;

        let tbl=this.container.querySelector("#table");
        
        this.container.querySelector("#glassForm").removeChild(tbl);
    ...
    inputDrink(a,b)
    {
        ...
        if(b>this.glasses){
            alert("Ne psotoji ta casa!");
            return;
        }

        let th2=this.container.querySelector("#th"+(b-1));
        
        let gl=this.container.querySelector("#canvas"+(b-1));
        ...
        this.currentOrders[this.num_order-1].updateGlass(b-1,this.currentDrinks[b-1]);
        ...
        this.currentOrders[this.num_order-1].setPrice(dr[1]);
    }
